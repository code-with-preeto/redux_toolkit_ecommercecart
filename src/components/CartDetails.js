...
import { addToCart,emptycartItem,removeSingleIteams,removeToCart } from '../redux/features/cartSlice';
      ...
      <div className='col-md-8 mt-5 mb-5 carddetails'>
        <div className='card'>
          <div className='card-header bg-dark p-3'>
            ...
            {
              cart.length>0?<button className='btn btn-danger mt-0 btn-sm' onClick={()=>{emptycart()}}><i className='fa fa-trash-alt mr-2'> EmptyCart</i></button>:""
            }
            </div>
            ...
            <tbody>
              {
                cart.map((data,index)=>{
                  return(
                    ...
                    <tr>
                      <td>
                        <button className='prdct-delete' onClick={()=>{
                          handleDecrement(data.id)
                        }}><i className='fa fa-trash-alt mr-2'></i></button>
                      </td>
                      <td><div className='product-img'><img src={data.imgdata} alt="" /></div>
                        </td>
                        <td><div className='product-img'><p>{data.dish}</p></div>
                        </td>
                        <td>{data.price}</td>
                        <td>
